...
function DrupalStrategy(options, verify) {
  ...
  // Parse URL of Drupal website
  var providerHost, providerProtocol;
  if (this._providerURL = url.parse(options.providerURL)) {
    providerProtocol = this._providerURL.protocol || 'http';
    providerHost = (this._providerURL.host + this._providerURL.pathname).replace(/\/$/, '');
  }

  // Determine all necessary OAuth options
  var oauthOptions = {
    requestTokenURL: url.format({protocol: providerProtocol, host: providerHost, pathname: 'oauth/request_token'}),
    accessTokenURL: url.format({protocol: providerProtocol, host: providerHost, pathname: 'oauth/access_token'}),
    userAuthorizationURL: url.format({protocol: providerProtocol, host: providerHost, pathname: 'oauth/authorize'}),
  ...
  this._resourceURL = url.format({protocol: providerProtocol, host: providerHost, pathname: options.resourceEndpoint});
  ...
  this._oauth.post(this._resourceURL, token, tokenSecret, {}, function (err, body, res) {
    ...
    try {
      ...
      var profile = {
        ...
        profileURL: url.format({protocol: providerURL.protocol || 'http', host: providerURL.host, pathname: providerURL.pathname + '/user/' + user.uid})
      };
